...
<DT>\link ranges::view::split_fn `view::split`\endlink</DT>
  <DD>Given a source range and a delimiter specifier, split the source range into a range of ranges using the delimiter specifier to find the boundaries. The delimiter specifier can be a value or a subrange. The delimiter character(s) are excluded from the resulting range of ranges.</DD>
