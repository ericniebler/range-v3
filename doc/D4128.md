...
- [Appendix 6: Implementation Notes](#appendix-6-implementation-notes)
    - [On Destinguishing Ranges from Non-Range Iterables](#on-destinguishing-ranges-from-non-range-iterables)
...
TODO Explain in detail how the iterator/sentinel abstraction can be used to model the three types of ranges, and how the algorithms use iterators/sentinels.
...
- Any algorithm that currently operates on a range denoted by two iterators gets an overload where the two iterator argument are replaced with a single Iterable argument. 
...
The Iterator/Sentinel abstraction is what makes it possible for the algorithms to handle these three cases with uniform syntax. However, the third option presents some challenges when trying to make some algorithms optimally efficient.

Counted ranges, formed by specifying a position and a count of elements, have iterators -- as all Iterables do. The iterators of a counted range must know the range's extent and how close they are to reaching it. Therefore, the counted range's iterators must store both an iterator into the underlying sequence and a count -- either a count to the end that is decremented each time the iterator is incremented, or a count from the front. Here is one potential design:
    ...
    {
        friend class counted_iterator<I>;
    ...
    {
        I it_;
        DistanceType<I> n_; // distance from front
    public:
        // ... constructors...
        ...
        friend bool operator==(counted_iterator const & it,
                               counted_sentinel const & end )
        {
            return it.n_ == end.count_;
        }
        // ... other operators...
    };
    ...
    template<WeakIterator I>
    struct common_type<counted_iterator<I>, counted_sentinel<I>>
        // ... see Appendix 2 ...

As you can see from the above code, when incrementing a `counted_iterator`, two increments happen: one to the underlying iterator, and the other to an internal count. This is fine and it works, but in some cases, it can be inefficient. Consider the trivial case of passing a `counted_iterator<list<int>::iterator>` to `advance`. Since the `counted_iterator` inherits its category from the underlying iterator, `counted_iterator<list<int>::iterator>` is bidirectional, and `advance` must increment the `counted_iterator` *N* times:
...
Notice that for each `++i` or `--i` here, *two* increments or decrements are happening when `I` is a `counted_iterator`. This is unnecessary. A better implementation for `counted_iterator` is:
...
This has a noticeable effect on the generated code. As it turns out, `advance` is one of the very few places in the standard library where it benefits from special handling of `counted_iterator`. Let's examine some algorithms to see why that's the case.
...
When passed counted iterators, at each iteration of the loop, we do two increments (for the underlying iterator and the count), and one comparison. Let's compare this against a hypothetical `for_each_n` algorithm that takes the underlying iterator and the count separately. It might look like this:
...
As we can see, for the hypothetical `for_each_n`, at each loop iteration, we do an increment, a decrement, and a comparison. That's exactly as many operations as `for_each` does when passed counted iterators. So a separate `for_each_n` algorithm is unnecessary if we have sentinels and `counted_iterator`s. This is going to be true for any algorithm that makes only one pass through the input range. That turns out to be great many of the algorithms.
...
### On Destinguishing Ranges from Non-Range Iterables
...
------------------------------------------|-----------------------|-----------------------
`decltype(*begin(declval<T&>()))`         | `value_type &`        | `[const] value_type &`
`decltype(*begin(declval<const T&>()))`   | `const value_type &`  | `[const] value_type &`

Notice how containers and ranges differ in the way a top-level cv-qualifier effects the reference type. Since a range is a proxy to elements stored elsewhere, a top-level `const` qualification on the *range* object has no effect at all on its iterator's reference type. But that's not true for a container that owns its elements.
