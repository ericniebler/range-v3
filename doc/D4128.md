...
The Range concept exists to give the range adaptors consistent and predictable semantics, and memory and performance characteristics. Since adaptors allow the composition of range objects, those objects must be efficiently copyable (or at lest movable). To be consistent and predictable, adaptors are only applicable to Ranges. Creating a Range from an Iterable type that is not a Range (e.g., `vector`) can formed from that type's `begin` and `end`.
