...
The assumption being that the operation is invalid if `rng1` and `rng2` refer to different elements. Although principled (for some set of principles), such a definition is almost certain to lead users into undefined behavior-land. Think of two type-erased `any_range`s which might have the same begin and end but refer to a different element sequence due to having been adapted by two different transformations.
...
To accomodate Iterables, the change would be as simple as:

    {
        ...
        auto __end = end-expr;  // can operator!= mutate end? -> const
...
// TODO BUGBUG this doesn't quite illustrate the point I'm trying to make. Imagine the result of upper_bound is *then* passed to an algorithm that calls distance. That's when counted iterators come out ahead.
    ...
    template<ForwardIterator I, Regular S>
        requires EqualityComparable<I, S> && Sortable<I> // from [N3351] [8]
