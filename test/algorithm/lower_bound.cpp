...
int main()
{
    ...
    CHECK(ranges::lower_bound_n(begin(a), size(a), a[0]) == &a[0]);
    CHECK(ranges::lower_bound_n(begin(a), size(a), a[1], less()) == &a[1]);
    CHECK(ranges::lower_bound_n(begin(a), size(a), 1, less(), &std::pair<int, int>::first) == &a[2]);
