...
int main()
{
    ...
    CHECK(ranges::lower_bound(ranges::make_iterator_range(a), 1, less(), &std::pair<int, int>::first).get_unsafe() == &a[2]);
    CHECK(ranges::lower_bound(ranges::make_iterator_range(c), 1, less(), &std::pair<int, int>::first).get_unsafe() == &c[2]);
