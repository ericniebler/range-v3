...
constexpr bool test_constexpr()
{
    ...
    int g[] = {0, 1, 2, 3, 4, 5};
    int h[] = {0, 1, 2, 3, 4, 5, 6};
    if(!(find_end(ia_b, ia_e, begin(b), b + 1).begin() == ia + sa - 1))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(c), c + 2).begin() == ia + 18))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(d), d + 3).begin() == ia + 15))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(e), e + 4).begin() == ia + 11))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(f), f + 5).begin() == ia + 6))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(g), g + 6).begin() == ia))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(h), h + 7).begin() == ia + sa))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_e, begin(b), b).begin() == ia + sa))
    {
        r = false;
    }
    if(!(find_end(ia_b, ia_b, begin(b), b + 1).begin() == ia))
    {
        r = false;
    }

    auto ir = make_subrange(ia_b, ia_e);
    if(!(find_end(ir, make_subrange(begin(b), b + 1)).begin() == ia + sa - 1))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(c), c + 2)).begin() == ia + 18))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(d), d + 3)).begin() == ia + 15))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(e), e + 4)).begin() == ia + 11))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(f), f + 5)).begin() == ia + 6))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(g), g + 6)).begin() == ia))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(h), h + 7)).begin() == ia + sa))
    {
        r = false;
    }
    if(!(find_end(ir, make_subrange(begin(b), b)).begin() == ia + sa))
    {
        r = false;
    }

    if(!(find_end(std::move(ir), make_subrange(begin(b), b + 1)).begin() == ia + sa - 1))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(c), c + 2)).begin() == ia + 18))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(d), d + 3)).begin() == ia + 15))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(e), e + 4)).begin() == ia + 11))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(f), f + 5)).begin() == ia + 6))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(g), g + 6)).begin() == ia))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(h), h + 7)).begin() == ia + sa))
    {
        r = false;
    }
    if(!(find_end(std::move(ir), make_subrange(begin(b), b)).begin() == ia + sa))
    {
        r = false;
    }

    auto er = make_subrange(ia_b, ia);
    if(!(find_end(er, make_subrange(b, b + 1)).begin() == ia))
    {
        r = false;
    }
    if(!(find_end(std::move(er), make_subrange(b, b + 1)).begin() == ia))
    {
        r = false;
    }
    return r;
