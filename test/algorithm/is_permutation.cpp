...
int main()
{
    ...
    // BoundedRange tests, with sentinels:
    {
        const int ia[] = {0, 1, 2, 3, 0, 5, 6, 2, 4, 4};
        const int ib[] = {4, 2, 3, 0, 1, 4, 0, 5, 6, 2};
        const unsigned sa = sizeof(ia)/sizeof(ia[0]);
        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const int*>(ia),
                                   sentinel<const int*>(ia + sa)),
                                   forward_iterator<const int*>(ib)) == true);

        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const int*>(ia),
                                   sentinel<const int*>(ia + sa)),
                                   ranges::range(forward_iterator<const int*>(ib),
                                   sentinel<const int*>(ib + sa))) == true);
        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const int*>(ia),
                                   sentinel<const int*>(ia + sa)),
                                   ranges::range(forward_iterator<const int*>(ib + 1),
                                   sentinel<const int*>(ib + sa))) == false);
        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const int*>(ia),
                                   sentinel<const int*>(ia + sa)),
                                   ranges::range(forward_iterator<const int*>(ib),
    ...
    // BoundedRange tests, with sentinels, with predicate:
    {
        const int ia[] = {0, 1, 2, 3, 0, 5, 6, 2, 4, 4};
        const int ib[] = {4, 2, 3, 0, 1, 4, 0, 5, 6, 2};
        const unsigned sa = sizeof(ia)/sizeof(ia[0]);
    ...
    // BoundedRange tests, with sentinels, with predicate and projections:
    {
        ...
        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const S*>(ia),
                                   sentinel<const S*>(ia + sa)),
                                   ranges::range(forward_iterator<const T*>(ib + 1),
                                   sentinel<const T*>(ib + sa)),
                                   std::equal_to<int const>(), &S::i, &T::i) == false);
        CHECK(ranges::is_permutation(ranges::range(forward_iterator<const S*>(ia),
                                   sentinel<const S*>(ia + sa)),
                                   ranges::range(forward_iterator<const T*>(ib),
