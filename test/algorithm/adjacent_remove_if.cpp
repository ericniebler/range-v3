...
test_range()
{
    int ia[] = {0, 1, 1, 1, 4, 2, 2, 4, 2};
    constexpr auto sa = ranges::size(ia);
    Iter r = ranges::adjacent_remove_if(::as_lvalue(ranges::make_subrange(Iter(ia), Sent(ia+sa))), ranges::equal_to{});
    CHECK(base(r) == ia + sa-3);
    CHECK(ia[0] == 0);
    CHECK(ia[1] == 1);
    CHECK(ia[2] == 4);
...
test_range_rvalue()
{
    ...
    ia[7].reset(new int(4));
    ia[8].reset(new int(2));
    Iter r = ranges::adjacent_remove_if(::as_lvalue(ranges::make_subrange(Iter(ia), Sent(ia+sa))), pred());
    CHECK(base(r) == ia + sa-2);
    CHECK(*ia[0] == 0);
    CHECK(*ia[1] == 1);
    CHECK(*ia[2] == 3);
...
int main()
{
...

    {
        ...
        auto r = ranges::adjacent_remove_if(std::move(ia), [](int x, int y) noexcept { return (x + y) % 2 == 0; }, &S::i);
        CHECK(::is_dangling(r));
        CHECK(ia[0].i == 0);
        CHECK(ia[1].i == 1);
        CHECK(ia[2].i == 2);
