...
test_range()
{
    ...
    constexpr unsigned sa = ranges::size(ia);
    int ib[sa];
    std::pair<InIter, OutIter> r = ranges::remove_copy_if(::as_lvalue(ranges::range(InIter(ia), Sent(ia+sa))), OutIter(ib), [](int i){return i == 2;});
    CHECK(base(r.first) == ia + sa);
    CHECK(base(r.second) == ib + sa-3);
    CHECK(ib[0] == 0);
    CHECK(ib[1] == 1);
...
int main()
{
    ...
    // Check initializer lists
    {
        ...
        int ib[sa];
        std::pair<const S*, int*> r = ranges::remove_copy_if({S{0}, S{1}, S{2}, S{3}, S{4}, S{2}, S{3}, S{4}, S{2}}, ib, [](int i){return i == 2;}, &S::i);
        CHECK(r.second == ib + sa-3);
        CHECK(ib[0] == 0);
        CHECK(ib[1] == 1);
        CHECK(ib[2] == 3);
