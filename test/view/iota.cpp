...
int main()
{
    ...
    CPP_assert(Same<std::int_fast32_t, range_difference_t<decltype(ushorts)>>);
    CPP_assert(Same<std::uint_fast32_t, range_size_t<decltype(ushorts)>>);
    ...
    CHECK(distance(sints.begin(), sints.end()) == (long) INT_MAX - (long) INT_MIN);
    CHECK(sints.size() == (unsigned)((long) INT_MAX - (long) INT_MIN));
