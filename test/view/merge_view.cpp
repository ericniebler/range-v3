...
//  Copyright Roi Barkan 2023-present
//
...
int main()
{
    ...
    {
        ::check_equal(views::merge(i1_infinite, i1_infinite) | views::stride(2) | views::take(100), i1_infinite | views::take(100));
        ::check_equal(views::merge(i1_infinite, i1_infinite) | views::drop(1) | views::stride(2) | views::take(100), i1_infinite | views::take(100));
    }

    // merge of two finite ranges/sets
    {
        auto res = views::merge(i1_finite, i2_finite);
        ...
        ::check_equal(res, {-3, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 4, 4, 4, 6, 9});

        // check if the final result agrees with the greedy algorithm
        std::vector<int> greedy_merge;
        merge(i1_finite, i2_finite, back_inserter(greedy_merge));
        ::check_equal(res, greedy_merge);
    ...
    // merge of two infinite ranges
    {
        auto res = views::merge(i1_infinite, i2_infinite);
        ...
        ::check_equal(res | views::take(9), {0, 0, 1, 3, 4, 6, 9, 9, 12});

        // check if the final result agrees with the greedy algorithm
        std::vector<int> greedy_merge;
        merge(i1_infinite | views::take(10), i2_infinite | views::take(10), back_inserter(greedy_merge));
        ::check_equal(res | views::take(16), greedy_merge | views::take(16));
    }

    // merge of a finite and an infinite range
    {
        auto res = views::merge(i1_finite, i2_infinite);
        ...
        ::check_equal(res | views::take(6), {0, 1, 1, 2, 2, 3});
    }

    // merge of an infinite and a finite range
    {
        auto res = views::merge(i1_infinite, i2_finite);
        ...
        ::check_equal(res | views::take(8), {-3, 0, 2, 3, 4, 4, 6, 6});
    }

    // merges involving unknown cardinalities
    {
        ...
        auto res1 = views::merge(i2_finite, rng0);
        ...
        auto res2 = views::merge(rng0, i2_finite);
        ...
        auto res3 = views::merge(i1_infinite, rng0);
        ...
        auto res4 = views::merge(rng0, i1_infinite);
        ...
        auto res5 = views::merge(rng0, rng0);
        ...
        ::check_equal(res5 | views::stride(2) | views::take(100), rng0 | views::take(100));
        ::check_equal(res5 | views::drop(1) | views::stride(2) | views::take(100), rng0 | views::take(100));
    }

    // test const ranges
    {
        auto res1 = views::merge(views::const_(i1_finite), views::const_(i2_finite));
        ...
        auto res2 = views::merge(views::const_(i1_finite), i2_finite);
    ...
    {
        auto res = views::merge(views::reverse(i1_finite), views::reverse(i2_finite), [](int a, int b)
        {
            return a > b;
        });
        ::check_equal(res, {9, 6, 4, 4, 4, 4, 4, 4, 3, 3, 3, 2, 2, 2, 1, -3});
    }
    ...
    {
        auto res = views::merge(b_finite, d_finite, [](const B& a, const D& b){ return a.val < b.val; });
        using R = decltype(res);
        ...
        ::check_equal(res, {B{-20}, B{-10}, B{0}, B{1}, B{2}, B{3}, B{3}, B{4}, B{6}, B{6}, B{8}, B{20}});
        auto it = begin(res);
    ...
    // projections
    {
        auto res1 = views::merge(b_finite, d_finite,
                                    less(),
                                    &B::val,
                                    &D::val
                                   );
        ...
        ::check_equal(res1, {B{-20}, B{-10}, B{0}, B{1}, B{2}, B{3}, B{3}, B{4}, B{6}, B{6}, B{8}, B{20}});

        auto res2 = views::merge(views::ints(-2, 10), b_finite,
                                    less(),
                                    identity(),
        ...
        ::check_equal(res2, {B{-20}, B{-10}, B{-2}, B{-1}, B{0}, B{1}, B{1}, B{2}, B{3}, B{3}, B{3}, B{4}, B{5}, B{6}, B{6}, B{7}, B{8}, B{8}, B{9}, B{20}});
    }
    ...
    // move
    {
        ...
        auto res = views::merge(v0, v1, [](const MoveOnlyString& a, const MoveOnlyString& b){return a<b;});

        std::vector<MoveOnlyString> expected;
        move(res, back_inserter(expected));

        ::check_equal(expected, {"a","b","b","c","x","x","y","z"});
        ::check_equal(v0, {"","","",""});
        ::check_equal(v1, {"","","",""});
    ...
    // iterator (in)equality
    {
        int r1[] = {1, 2, 3};
        int r2[] = {   2, 3, 4, 5};
        auto res = views::merge(r1, r2); // 1, 2, 2, 3, 3, 4, 5

        auto it1 = ranges::next(res.begin(), 5); // *it1 == 4, member iterator into r1 points to r1.end()
