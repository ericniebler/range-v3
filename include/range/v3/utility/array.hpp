...
#ifndef RANGES_V3_UTILITY_ARRAY_HPP
#define RANGES_V3_UTILITY_ARRAY_HPP
...
namespace ranges
{
    inline namespace v3 {
        ...
        struct array
        {
            ...
            RANGES_RELAXED_CONSTEXPR array() = default;

            RANGES_RELAXED_CONSTEXPR void fill(const value_type& u)
            {
                ranges::fill_n(elems_, N, u);
            }
            RANGES_RELAXED_CONSTEXPR
            void swap(array& a) noexcept(is_nothrow_swappable<T, T>::value)
            {
            ...
            // element access:
            RANGES_RELAXED_CONSTEXPR reference operator[](size_type n)
            {
                return elems_[n];
            }
            RANGES_RELAXED_CONSTEXPR const_reference operator[](size_type n) const
            {
                return elems_[n];
            }

            RANGES_RELAXED_CONSTEXPR reference at(size_type n)
            {
                if (n >= N)
                    throw std::out_of_range("array::at");
                return elems_[n];
            };
            RANGES_RELAXED_CONSTEXPR const_reference at(size_type n) const
            {
                if (n >= N)
                    throw std::out_of_range("array::at");
                return elems_[n];
            ...
            RANGES_RELAXED_CONSTEXPR reference front()
            {
                return elems_[0];
            }
            RANGES_RELAXED_CONSTEXPR const_reference front() const
            {
                return elems_[0];
            }
            RANGES_RELAXED_CONSTEXPR reference back()
            {
                return elems_[N > 0 ? N-1 : 0];
            }
            RANGES_RELAXED_CONSTEXPR const_reference back() const
            {
                return elems_[N > 0 ? N-1 : 0];
            }
        ...
        template <class T, size_t N>
        RANGES_RELAXED_CONSTEXPR
        auto swap(const array<T, N>& x, const array<T, N>& y)
                noexcept(is_nothrow_swappable<T, T>::value)
                -> typename std::enable_if<is_swappable<T, T>::value, void>::type
        {
            x.swap(y);
        }
...
template <class T, size_t N>
class tuple_size<ranges::array<T, N>>
    : public integral_constant<size_t, N> {};

template <size_t I, class T, size_t N>
class tuple_element<I, ranges::array<T, N> >
{
 public:
    using type = T;
};
...
#endif // RANGES_V3_UTILITY_ARRAY_HPP
