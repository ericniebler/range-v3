    ...
    inline namespace v3
    {
        ...
        namespace view
        {
            ...
            private:
                ...
                struct subrange_pred
                {
                    ...
                    subrange_pred(Sub && sub)
                      : sub_(all(RANGES_FORWARD(sub))), len_(distance(sub_))
                    {}
                    std::pair<bool, iterator_t<Rng>>
                    operator()(iterator_t<Rng> cur, sentinel_t<Rng> end) const
                    {
            ...
            public:
                ...
                {
                    return {all(RANGES_FORWARD(rng)), predicate_pred<Rng, Fun>{std::move(fun)}};
                }
                template<typename Rng,
