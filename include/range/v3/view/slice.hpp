    ...
    inline namespace v3
    {
        ...
        namespace view
        {
            ...
            private:
                ...
                {
                    return {all(RANGES_FORWARD(rng)), from, count};
                }
                template<typename Rng,
            ...
            public:
                ...
                auto operator()(Rng && rng, range_difference_type_t<Rng> from,
                    ...
                    decltype(slice_fn::invoke_(RANGES_FORWARD(rng), from, to - from,
                        range_concept<Rng>{}))
                {
                    RANGES_EXPECT(0 <= from && from <= to);
                    return slice_fn::invoke_(RANGES_FORWARD(rng), from, to - from,
                        range_concept<Rng>{});
                }
                // slice(rng, 4, end-2)
                ...
                auto operator()(Rng && rng, range_difference_type_t<Rng> from,
                    ...
                    decltype(slice_fn::invoke_(RANGES_FORWARD(rng), from,
                        distance(rng) + to.dist_ - from, range_concept<Rng>{}))
                {
                    ...
                    return slice_fn::invoke_(RANGES_FORWARD(rng), from,
                        distance(rng) + to.dist_ - from, range_concept<Rng>{});
                }
                ...
                auto operator()(Rng && rng, detail::from_end_<range_difference_type_t<Rng>> from,
                    detail::from_end_<range_difference_type_t<Rng>> to) const ->
                    decltype(slice_fn::invoke_(RANGES_FORWARD(rng), from.dist_,
                        ...
                        bounded_range_concept<Rng>{}()))
                {
                    ...
                    return slice_fn::invoke_(RANGES_FORWARD(rng), from.dist_,
                        to.dist_ - from.dist_, range_concept<Rng>{},
                        bounded_range_concept<Rng>{}());
                }
                // slice(rng, 4, end)
                ...
                auto operator()(Rng && rng, range_difference_type_t<Rng> from, end_fn) const ->
                    decltype(ranges::view::drop_exactly(RANGES_FORWARD(rng), from))
                {
                    RANGES_EXPECT(0 <= from);
                    return ranges::view::drop_exactly(RANGES_FORWARD(rng), from);
                }
                // slice(rng, end-4, end)
                template<typename Rng,
                    CONCEPT_REQUIRES_((InputRange<Rng>() && SizedRange<Rng>()) ||
