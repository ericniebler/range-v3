...
namespace ranges
{
    ...
    namespace detail
    {
        using span_size_t = std::size_t;
    } // namespace detail
    /// \endcond

    constexpr detail::span_size_t dynamic_extent = SIZE_MAX;
    ...
    namespace detail
    {
        ...
        template<typename T>
        constexpr span_size_t byte_size(span_size_t n) noexcept
        {
            return n == dynamic_extent ? dynamic_extent
                                       : (RANGES_EXPECT(n <= SIZE_MAX / sizeof(T)),
                                          n * narrow_cast<span_size_t>(sizeof(T)));
        }
        ...
        struct span_extent
        {
            ...
            constexpr span_extent(span_size_t size) noexcept
            ...
            constexpr span_size_t size() const noexcept
            {
                return N;
            }
        ...
        constexpr span_size_t subspan_extent(span_size_t Extent, span_size_t Offset,
    ...
    template<typename Rng, detail::span_size_t N>
    CPP_concept span_dynamic_conversion =
        N == dynamic_extent ||
            static_cast<detail::span_size_t>(range_cardinality<Rng>::value) < cardinality();

    template<typename Rng, detail::span_size_t N>
    CPP_concept span_static_conversion =
        N != dynamic_extent && static_cast<detail::span_size_t>(range_cardinality<Rng>::value) == N;
    // clang-format on
    /// \endcond

    template<typename T, detail::span_size_t N = dynamic_extent>
      ...
      , public detail::span_extent<N>
    {
        ...
        using size_type = detail::span_size_t;
        ...
        constexpr span(pointer ptr, size_type cnt) noexcept
        ...
        constexpr span(pointer first, pointer last) noexcept
          : span{first, (RANGES_EXPECT(static_cast<size_type>(last - first) >= 0),
        ...
        constexpr span(Rng && rng) noexcept(noexcept(ranges::data(rng),
                                                        ranges::size(rng)))
          : span{ranges::data(rng), detail::narrow_cast<size_type>(ranges::size(rng))}
        {}
        ...
        constexpr span<T> first(size_type cnt) const noexcept
        {
        ...
        constexpr span<T> last(size_type cnt) const noexcept
        {
            return RANGES_EXPECT(cnt <= size()),
        ...
        constexpr span<T, dynamic_extent> subspan(size_type offset) const noexcept
        {
            return RANGES_EXPECT(size() >= offset),
                   RANGES_EXPECT((offset == 0 && size() == 0) || data_ != nullptr),
                   span<T, dynamic_extent>{data_ + offset, size() - offset};
        }
        constexpr span<T, dynamic_extent> subspan(size_type offset, size_type cnt) const
            noexcept
        {
    ...
    template<typename T, detail::span_size_t N>
    RANGES_INLINE_VAR constexpr bool enable_borrowed_range<span<T, N>> = true;

    template<typename T, detail::span_size_t N>
    constexpr detail::span_size_t span<T, N>::extent;
    ...
    template(typename Rng)(
        ...
        span(Rng && rng)
            ->span<detail::element_t<Rng>, (static_cast<detail::span_size_t>(range_cardinality<Rng>::value) < cardinality()
                                                ? dynamic_extent
                                                : static_cast<detail::span_size_t>(
    ...
    constexpr span<ElementType> make_span(ElementType * ptr,
                                          detail::span_size_t cnt) noexcept
    {
            ...
            noexcept(ranges::data(rng), ranges::size(rng)))
    {
        return {ranges::data(rng),
                detail::narrow_cast<detail::span_size_t>(ranges::size(rng))};
    }
    template(typename Rng)(
        /// \pre
        requires contiguous_range<Rng> AND
    ...
    {
        return {ranges::data(rng), static_cast<detail::span_size_t>(range_cardinality<Rng>::value)};
    }
