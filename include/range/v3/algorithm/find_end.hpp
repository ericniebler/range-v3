    ...
    inline namespace v3
    {
        struct end_finder : bindable<end_finder>
        {
            ...
            invoke(end_finder find_end, ForwardRange2 && rng2)
                -> decltype(find_end.move_bind(std::placeholders::_1, std::forward<ForwardRange2>(rng2)))
            {
                CONCEPT_ASSERT(ranges::ForwardRange<ForwardRange2>());
                return find_end.move_bind(std::placeholders::_1, std::forward<ForwardRange2>(rng2));
            }
            ...
            invoke(end_finder find_end, ForwardRange2 && rng2, BinaryPredicate pred)
                -> decltype(find_end.move_bind(std::placeholders::_1, std::forward<ForwardRange2>(rng2), std::move(pred)))
            {
                CONCEPT_ASSERT(ranges::ForwardRange<ForwardRange2>());
                return find_end.move_bind(std::placeholders::_1, std::forward<ForwardRange2>(rng2), std::move(pred));
            }
        };
