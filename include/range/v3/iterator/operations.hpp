...
namespace ranges
{
    ...
    struct advance_fn
    {
        ...
        template<typename I>
        constexpr auto operator()(I &i, iter_difference_t<I> n) const -> CPP_ret(void)( //
            requires Iterator<I>)
    ...
    {
        template<typename I>
        constexpr auto operator()(I it) const -> CPP_ret(I)(requires Iterator<I>)
        {
            return ++it;
        }
        template<typename I>
        ...
        template<typename I, typename S>
        constexpr auto operator()(I it, S s) const -> CPP_ret(I)(requires Sentinel<S, I>)
        {
    ...
    public:
        ...
        template<typename Rng>
        constexpr auto operator()(Rng &&rng) const -> CPP_ret(range_difference_t<Rng>)( //
            requires Range<Rng>)
        {
            // Better not be trying to compute the distance of an infinite range:
            RANGES_EXPECT(!is_infinite<Rng>::value);
