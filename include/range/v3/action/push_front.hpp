    ...
    inline namespace v3
    {
        ...
        namespace adl_push_front_detail
        {
            ...
            auto push_front(Cont && cont, T && t) ->
                decltype((void)unwrap_reference(cont).push_front(RANGES_FORWARD(t)))
            {
                unwrap_reference(cont).push_front(RANGES_FORWARD(t));
            }

            template<typename Cont, typename Rng,
                CONCEPT_REQUIRES_(LvalueContainerLike<Cont>() && Range<Rng>())>
            auto push_front(Cont && cont, Rng && rng) ->
                decltype((void)ranges::insert(cont, begin(cont), RANGES_FORWARD(rng)))
            {
                ranges::insert(cont, begin(cont), RANGES_FORWARD(rng));
            }
            ...
            public:
                ...
                {
                    push_front(rng, RANGES_FORWARD(t));
